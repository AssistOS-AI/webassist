...
- Manifest: `webassist/webAssist/manifest.json`
- Role: Guest-enabled visitor assistant for standalone webassist deployments.
- Authentication: Manifest-level `guest: true` means normal Ploinky guest policy applies and visitor operations must enforce guest limitations locally. Manifest guest: true.
...
- Persistent state: Visitor/session/lead data must remain under the configured data store and must not leak to static assets or logs. Manifest volumes: {".ploinky/repos/webassist/data":"/data"}.
...
`webassist/webAssist` remains compatible with Ploinky only while it preserves router-mediated entry, secure-wire invocation, scoped guest behavior, explicit manifest-declared HTTP services, workspace-confined storage, redacted logging, and local domain authorization. Any source change that affects these contracts must update this specification, the local docs, and the local guide files in the same change set.
